...
class AlarmBroadcast : BroadcastReceiver() {
    ...
    companion object {
        ...
        private val preferencesRepository = PrefsReporitory
        ...
        private fun doDoStartAlarm(context: Context) {
            val timeFromFirstLaunch = System.currentTimeMillis() - preferencesRepository.firstLaunchMillis
            if (timeFromFirstLaunch > TimeUnit.DAYS.toMillis(3)) return

            val am = context.getSystemService(AlarmManager::class.java)

            val delay = if (preferencesRepository.sentFirstNotification)
                TimeUnit.MINUTES.toMillis(10)
            else {
                preferencesRepository.sentFirstNotification = true
